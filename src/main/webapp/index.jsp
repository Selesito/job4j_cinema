...
<head>
    ...
    <script>
        ...
        function show() {
            $.ajax({
                type: 'GET',
                url: 'http://localhost:8080/job4j_cinema_war_exploded/hall',
                dataType: 'json'
            }).done(function(data) {
                $('#table').empty();
                for (let row = 1; row <= 3; row++) {
                    ...
                    for (let cell = 1; cell <= 3; cell++) {
                        ...
                        for (let i = 0; i < data.length; i++) {
                            if (data[i].id === ids) {
                                section += '<td style="color:#FF0000" id="' + data[i].id + '"><input disabled="disabled" ' +
                                    'type="checkbox" name="place" value="' + data[i].id + '">     Место занято! </td>';
                                added = false;
                                break;
                            }
                        }
